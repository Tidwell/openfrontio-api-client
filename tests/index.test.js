...
describe("OpenFront API Client", () => {
  ...
  describe("getGames", () => {
    ...
    it("should handle malformed pagination headers", async () => {
      const mockGames = [{ game: "game123", type: "Public" }];
      mockApiResponse(mockGames, 200, { "content-range": "invalid" });
      ...
      expect(result.range.end).toBe(0);
    });

    it("should handle missing pagination headers", async () => {
      const mockGames = [{ game: "game123", type: "Public" }];
      mockApiResponse(mockGames, 200);
  ...
  describe("getPlayerSessions", () => {
    ...
    it("should return BigInts when useBigInt is true", async () => {
      ...
      expect(result[0].gameId).toBe(BigInt(hugeId));
    });
  });
  ...
  describe("getClanStats", () => {
    ...
    it("should filter out null and undefined parameters", async () => {
      ...
      const callArgs = https.request.mock.calls[0][0];
      expect(callArgs.path).toContain("valid=value");
      expect(callArgs.path).not.toContain("nullParam");
      expect(callArgs.path).not.toContain("undefinedParam");
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle 500 status code", async () => {
      ...
      await expect(
        api.getPlayerInfo({ playerId: "player1" }),
      ).rejects.toMatchObject({
        statusCode: 500,
    ...
    it("should default to 500 if status code is missing", async () => {
      const req = new EventEmitter();
      req.end = vi.fn();

      const res = new EventEmitter();
      // res.statusCode is undefined
